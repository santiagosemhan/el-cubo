...
const CharacterPage = ({ character, node, bgImage }) => {
    ...
    return (
        <AppLayout onlyContent>
            ...
            <BackToCharacters />
            <CharacterIndex character={character} node={node} bgImage={bgImage} title={''} />
...
export const getStaticProps: GetStaticProps = async (context) => {
    ...
    const characterNode = initialNodes.filter(node => {
        ...
        return nodeData[0].character_name.split(' ').slice(-1).join(' ').trim().toLowerCase() === context.params.personaje;
    });
